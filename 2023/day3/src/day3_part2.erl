...
get_adjacent_numbers(0, Gear, Lines) ->
    ...
    lists:merge(
        numbers_with_line_pos(Current, NCurrent),
        numbers_with_line_pos(Below, NBelow)
    );
get_adjacent_numbers(N, Gear, Lines) when N =:= map_size(Lines)-1 ->
    ...
    NCurrent = get_current_numbers(Gear, Current#line.numbers),
    lists:merge(
        numbers_with_line_pos(Above, NAbove),
        numbers_with_line_pos(Current, NCurrent)
    );
...
numbers_with_line_pos(Line, Numbers) ->
    LPos = Line#line.pos,
    lists:map(fun(Number) -> {LPos, Number} end, Numbers).
...
intersect_current({NPos, NLen}, SPos) ->
    SPos =:= NPos-1 orelse SPos =:= NPos+NLen.

intersect_above_or_below({NPos, NLen}, SPos) ->
